...
# Show the two confirmed winning trades: Jul 28 (Long) and Aug 18 (Short)
FOCUS_START  = '2016-07-20'
FOCUS_END    = '2016-09-10'
# Signals: look back to Jul 01 so episodes before window still project their lines
...
for sig_idx, (ts, row) in enumerate(bull_sigs.iterrows()):
    ...
    tp_measured = row.get('trap_target_measured', np.nan)
    ...
    # ── TP MEASURED ──
    if pd.notna(tp_measured):
        fig.add_shape(type="line", x0=ts_left, x1=ts_right, y0=tp_measured, y1=tp_measured,
                      line=dict(color="#d500f9", width=2, dash="dash"))
        fig.add_annotation(x=ts_right, y=tp_measured, text=f"TP MEASURED  {tp_measured:.5f}",
                           showarrow=False, font=dict(size=9, color="#d500f9", family=FONT_FAMILY),
                           xanchor="left", bgcolor="rgba(15,23,42,0.85)")
...
fig.update_layout(
    title=dict(text="AUDUSD Month 2 Video 8 — Measured Move Target Projections", font=dict(size=14, color="#f8fafc", family=FONT_FAMILY)),
...
OUTPUT_FILE = "ICT_MONTH2_VIDEO8_MEASURED_MOVE.html"
