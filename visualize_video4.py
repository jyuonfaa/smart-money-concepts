...
def run_video4_triple():
    symbol = "EURUSD=X"
    output_name = "ICT_VIDEO_4_TRIPLE.html"

    print(f"Generating Triple Timeframe Audit...")
    ...
    for i in range(1, len(d_swings)):
        ...
        # --- SENIOR UPGRADE: PREMIUM STOP RUN (WIDER SWING) ---
        if status == "INVALIDATED" and i >= 3:
            p_minus_2 = d_swings.iloc[i-3]
    ...
    for i, df_plot in enumerate([df_1d, df_4h, df_15m]):
        fig.add_trace(go.Candlestick(x=df_plot.index, open=df_plot['open'], high=df_plot['high'], low=df_plot['low'], close=df_plot['close'], name=f"P{i+1}"), row=i+1, col=1)
    ...
    # Track last fire time per zone+type to enforce 8-candle (2 hour) cooldown
    # Key: (zone_id, sig_type) -> last_fire_time
    zone_cooldowns = {}

    for _, row in m15_swings.iterrows():
        for i, ote in enumerate(active_otes):
            if min(ote['62'], ote['79']) <= row['p'] <= max(ote['62'], ote['79']):
                # Cooldown check: 8 candles (2 hours on 15M)
                cooldown_key = (i, row['type'])
                if cooldown_key in zone_cooldowns:
                    time_diff = row['ts'] - zone_cooldowns[cooldown_key]
                    if time_diff < pd.Timedelta(hours=2):
                        continue # Skip: visual stacking detected

                all_candidates.append((row['ts'], row['p'], row['type'], i, row['label']))
                zone_cooldowns[cooldown_key] = row['ts']
                break 

    # Step 2 — Sort the full list by timestamp
    all_candidates.sort(key=lambda x: x[0])

    # Step 3 — Apply ONE global alternation filter to the sorted list
    last_type = None
    final_signals = []
    ...
    # Step 4 — Render ONLY from final_signals
    ...
    # Step 5 — Verify
    print("\n" + "="*40)
    print(f"VERIFICATION: DEFINITIVE FIX")
    ...
    assert len(plotted_signals) == len(final_signals), \
        f"COUNT MISMATCH: chart={len(plotted_signals)}, filtered={len(final_signals)}"

    # Check sequence
    for i in range(1, len(plotted_sequence)):
        assert plotted_sequence[i] != plotted_sequence[i-1], \
            f"SEQUENCE VIOLATION on chart at index {i}: {plotted_sequence}"
    ...
    fig.update_layout(height=1600, template="plotly_dark", title="ICT VIDEO 4: SENIOR INSTITUTIONAL AUDIT", showlegend=False, xaxis1_rangeslider_visible=False, xaxis2_rangeslider_visible=False, xaxis3_rangeslider_visible=False)
    fig.write_html(output_name)
    print(f"\n[DONE] TRIPLE AUDIT SUCCESS: OPEN '{output_name}'")
