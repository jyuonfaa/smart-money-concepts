"""
ICT Video 4: GOLD FRACTAL AUDIT
Verification of OTE respect on XAUUSD
"""
import pandas as pd
...
def run_gold_audit():
    ...
    output_name = f"ICT_GOLD_AUDIT_{uid}.html"

    print(f"Generating GOLD Institutional Audit [{uid}]...")
    ...
    fig = make_subplots(rows=3, cols=1, vertical_spacing=0.03, row_heights=[0.3, 0.3, 0.4],
                        subplot_titles=("GOLD DAILY", "GOLD 4H (Interbank)", "GOLD 15M (Surgical)"))
    ...
    fig.update_layout(height=1600, template="plotly_dark", title="GOLD INSTITUTIONAL FRACTAL AUDIT", showlegend=False, xaxis1_rangeslider_visible=False, xaxis2_rangeslider_visible=False, xaxis3_rangeslider_visible=False)
    fig.write_html(output_name)
    print(f"\n[DONE] GOLD SUCCESS: OPEN '{output_name}'")
